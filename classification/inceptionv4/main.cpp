...
void load_image_and_preprocess()
{
  ...
  // resize image to 299 * ? or ? * 299
  cv::Mat resized_img;
  float ratio = 299 / fmin(origin_img.size().width, origin_img.size().height);
  cv::resize(origin_img, resized_img, cv::Size(), ratio, ratio);

  // Crop image center 299 * 299
  ...
  // mean
  float_img -= Scalar(127.5, 127.5, 127.5);
...
{
  uint32_t i = 0, size = 1001;
  uint32_t cls[5];
  float prob[5];

  float* output_data = get_data_from_file("input_img.bin_output0_1_1001.txt", 1001);
