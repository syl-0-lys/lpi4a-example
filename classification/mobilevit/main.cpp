...
void load_image_and_preprocess()
{
  ...
  // resize image to 256 * ? or ? * 256
  cv::Mat resized_img;
  float ratio = 256 / fmin(origin_img.size().width, origin_img.size().height);
  cv::resize(origin_img, resized_img, cv::Size(), ratio, ratio);

  // Crop image center 256 * 256
...
int main()
{
  ...
  std::cout << " ********** run mobilevit **********" << std::endl;
