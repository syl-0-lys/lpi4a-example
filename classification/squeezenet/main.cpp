...
void load_image_and_preprocess()
{
  ...
  // resize image to 227 * ? or ? * 227
  cv::Mat resized_img;
  float ratio = 227 / fmin(origin_img.size().width, origin_img.size().height);
  cv::resize(origin_img, resized_img, cv::Size(), ratio, ratio);

  // Crop image center 227 * 227
  ...
  // mean
  float_img -= Scalar(104, 117, 124);

  // scale
  float_img *= 1;

  // save to file
  FILE* fp = fopen("data_img.tensor", "w");
  FILE* bfp = fopen("data_img.bin", "w");
...
void load_result_and_postprocess()
{
  ...
  float* output_data = get_data_from_file("data_img.bin_output0_1_1000_1_1.txt", 1000);
...
int main()
{
  ...
  system("./hhb_out/hhb_runtime ./hhb_out/hhb.bm data_img.bin");
